...
Again, what counts as "similar predictions" is specified as part of the definition of the prediction task at hand, the relevant prediction class.[^18] And, for probabilistic predictions in which success is no longer an all-or-nothing matter, the $J$ prediction tasks for which the outcome is already known may be weighted according to their similarity to the task of interest, specifying $w_j$ for $j=1, ..., J$ (with $\sum w_j = 1$) to define a *weighted prediction class*. Equal weights may be appropriate in certain circumstances, but generally not if the prediction task of interest is not a random sample from the same distribution of tasks that generated the known outcomes. Then the weights should be used to better estimate the model's predictive accuracy for the task at hand, as illustrated in the examples below.
...
Define weighted prediction class, the cross-validation prediction tasks most like the task of interest.
