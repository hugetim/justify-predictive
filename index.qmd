...
Many machine learning researchers already seem to regard successful predictions (under the rubric of "cross-validation" and related approaches) as a direct rationale for using a given model, so that embracing the argument here would have little or no impact on their practice. Others (such as most economists, in my experience) regard the use of cross-validation for model selection as an ad hoc approach that could only be justified in terms of more traditional statistics concepts, if at all.

In the philosophy literature there are close relatives but, so far as I know, no precedent for directly justifying the use of successful predictive methods, in themselves. Schurz's meta-inductive program [@schurz2019] justifies a meta-level strategy by optimality theorems---asymptotically, and relative to a pool of competitors; the present argument is object-level, finite-data, and eliminative, and @sec-schurz argues that this difference is not optional, because optimality theorems cannot carry the forward-looking warrant at issue. Hitchcock and Sober's analysis of novel prediction as a guard against overfitting [@hitchcock2004] is perhaps the nearest published relative of @sec-under, and Mayo's severity requirement [@mayo2018] makes a kindred demand on the evidential weight of accommodations. What appears to be missing---the gap this paper aims to fill---is the direct question: what justifies using the method with the successful record for the very next prediction? As a recent review article on "Philosophy of Statistics" puts it, in a section discussing some approaches to model selection: "methods that use cross-validation...have, unduly, not received as much attention in the philosophical literature."[^1]
