...
More generally though, supposing a line with non-zero slope were required (or a method more complicated still), what if there were another predictive method that could have plausibly predicted any held out point? We cannot be sure there is a unique fully-utilized predictive method capable of predicting any held-out subset of the data. If an alternative would also make exactly the same predictions for all unknown tasks in the prediction class, that is of no real concern because it is the predictions themselves that we care about. But we cannot rule out a non-equivalent alternative, a predictive method that is equally successful so far but potentially differs in its further predictions.
