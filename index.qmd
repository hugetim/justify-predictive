...
abstract: There is a pragmatic justification for expecting a predictive method to perform the same in the future as in the past. This justified expectation of continued predictive ability resolves Hume's problem of induction.
...
As another example, consider the non-linear prediction task presented in [@forster2000]. Suppose you observe the values of a function at evenly spaced intervals between 0 and 3.5, the values shown in @fig-known-data, and you want to predict the values between 3.5 and 5.

{{< embed notebooks/forster2000_adapted.ipynb#fig-known-data >}}

And suppose your candidate methods for doing so are using Gaussian polynomial models up to degree 4, with the question being which degree to use. Define weighted prediction class, the cross-validation prediction tasks most like the task of interest.
