...
Suppose we have data on age, sex, and height for a random sample of those aged 20-74 in a given population.[^2a] And suppose the goal is to predict the height of someone aged 20-74, chosen at random.[^3a] Leave-one-out (LOO) cross-validation is the natural choice for quantifying predictive performance in this context.[^4a] We consider a range of predictive methods, which each involve estimating, by ordinary least squares, a statistical model in which height is specified as a function of age and sex. The most successful method is the specification ‘*female age age2*’. In other words, projecting height onto a linear combination of gender, age and age squared performs best for predicting height. So that is the model we should use to predict the height of someone aged 20-74 in this population.
...
### Potential for incommensurability

In @sec-prag, we considered the definition of what counts as "similar predictions" in @prp-same as part of the predictive method in question. This presents a potential complication when it comes to comparing the success of predictive methods, as in @prp-best, as opposed to assessing the success of a single method as in @prp-worked. Rival predictive methods may conceptualize the data differently, so that certain predictions considered similar to the target prediction by method A may not be considered such by B, and vice versa. Or they may have different input data requirements, so that method B may not capable of producing predictions for some of the scenarios in method A's track record.

In some cases, method A may preform better on both its own "similar predictions" and those prescribed by method B. Or the "similar predictions" prescribed by the two methods may overlap, enabling a comparison on the intersection. Otherwise, though, @prp-best is not capable of specifying a single best predictive method. 
