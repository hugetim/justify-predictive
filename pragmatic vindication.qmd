...
Again, what counts as "similar predictions" is specified as part of the definition of the prediction task at hand, the relevant prediction class.[^18] In addition, for probabilistic predictions, the $J$ prediction tasks for which the outcome is already known may be weighted according to their similarity to the task at hand, specifying $w_j$ for $j=1, ..., J$. Equal weights may be appropriate in certain circumstances, but generally not if the prediction task at hand is other than a random sample of the same distribution of tasks that generated the known outcomes. Then the weights should be used to better estimate the model's predictive accuracy for the task at hand, as discussed in an example with real data below.

[^18]: This setup is closely related to what is sometimes referred to as the Common Task Framework in the machine learning literature.
