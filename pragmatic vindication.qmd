...
-   Add a numerical sine curve fitting example (or zigzag: $f(x) = \left| \left( \frac{2}{\lambda} \cdot x \bmod 2 \right) - 1 \right|
$), in which linear is best (perhaps even without added random noise)
...
[^15a]: The (relatively simple) model selected by LOO R^2^ runs counter to this pattern. (And this disconnect underscores the importance of differentiating between within-sample prediction and generalization when assessing a model specification.)
