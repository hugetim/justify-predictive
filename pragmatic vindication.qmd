...
To be clear, though, using a data point to calculate the value of _that same data point_ is not a prediction, at least as I use the term herein. (That should go without saying, but the term "predict" is abused in this way in certain statistical contexts.)
...
The argument proceeds as follows. @sec-under shows how thinking in terms of predictive methods alleviates the problem of underdetermination. Given that, @sec-prag lays out a pragmatic justification for the case of deterministic predictions. @sec-prob extends that justification to probabilistic predictions. @sec-induct addresses the major arguments against Reichenbach's pragmatic justification of induction, which is closely related to this argument. @sec-schurz relates the framework of this paper to a prominent recent attempt to justify induction with reference to prediction. The final section concludes by sketching out some potential implications for philosophy of probability, statistical inference, and scientific explanation.
...
Now, one potential objection to the claim that thinking in terms of predictive methods distinguishes the best-fit line from all other curves that pass through the 10 known data points is what @forster1994 termed the "subfamily problem." Take any one of those innumerable curves that pass through the 10 known points but are not straight lines, and suppose someone proposed one of those as a "predictive method." It would perform just as well on all 100 of the predictive tasks in our specified class with known outcomes we can check. The problem, of course, is that it is simply not plausible that someone would have been able to pick such a curve without knowing all 10 outcomes,^[When plausibility is in doubt, a literal temporal predictions can settle the matter, in principle. But a mere thought experiment about temporal predictions should hopefully be sufficient, typically.] without some "leakage" of information about the supposedly predicted outcome.^[The term "leakage" is used to refer to "the introduction of information about the target...that should not be legitimately available" in the machine learning literature. KaufmanEtAl2012]

Consider a more concrete form of our example: An elementary student has been given an assignment to weigh an item 10 times and plot the weight against the time of day. She chooses a 3 lb. dumbbell and weighs it on a balance, using a set of standard weights that allow her to determine its weight with a precision of 0.1 lb (and a range from 0 to 10). The result: 3.0 lbs. She weighs it seven more times throughout the day and finds the same result each time. Suppose she is sufficiently clever and whimsical to imagine a curve that fits those eight data points exactly but varies wildly in between them. On the one hand, there are a huge number of possible curves that would turn out to successfully predict her 9th measurement exactly.^[Discretizing the x-axis to even seconds, we can put a number on it: there are $100^{3600*24-9}=10^{172782}$ such curves that go through all nine points exactly.] On the other hand, only one in 100 such curves will prove successful (on that single prediction task). The former is how the situation looks from the standpoint of accomodation, the latter from the perspective of prediction.

Somewhat related is the challenge of ruling out "grue"-like predicates: how does thinking in terms of predictive methods distinguish the best-fit line from predicting a weight of "thrine"? All fluent speakers of the "grue" language know that "thrine" means the dumbbell will weigh in at 3 until after 11pm, at which time it will be 9.^[For the uninitiated, this is Nelson Goodman’s "new riddle of induction."] This predictive method performs flawlessly on all 100 predictive tasks and would continue its success in any other predictions through well after the student's bedtime. Why should we favor the best-fit line over it (not to mention the innumerable alternatives: threven, threight, thren, etc.) for predictions about the last hour of the day? 
...
In other words, we ought to *act as if there were* uniformity of nature in a specific sense: that a uniquely successful predictive method will continue to work. Prediction (and thus purposeful action) is otherwise not possible for the prediction class in question. One can choose to act in this way (or consciously assent in good faith to one’s natural inductive tendencies) despite intellectually acknowledging not knowing whether nature is reliably uniform in any sense. There’s nothing to lose by acting in this way, and there is potentially something to gain. If past experience offers usable information, this approach will use it.[^13] If not, purposeful action was impossible anyway.

[^13]: Suppose there were some other way of identifying a reliable predictive method: using a Ouija board, say. Such a "meta" method can itself be considered a predictive method. And if the Ouija board really did reveal reliable predictive methods, the methods it dictates would work reliably.
...
In this respect, there is a strong parallel with Karl Popper's conception of falsification as the goal of science. But Popper explicitly denies giving an account of rational prediction.[^17] He also focused on theories as the unit of analysis rather than predictive methods, for which it is more conceivable that there may be just one that has proven successful (so far).
...
[^20]: Indeed, as we shall see below, the most successful method will itself generally imply that another method may well have been chosen if the past predicted outcomes had turned out differently, if different values of the predicted probability distribution had been realized.

The idea is that, when a successful deterministic predictive method is unavailable, we still ought to act as if there were uniformity of nature in the following sense: that a predictive method exists which reliably indicates the true probabilities of the outcome in question.[^21] Here, "true" means the best probability distribution possible to predict (given the allowed inputs of the predictive task), where "best" means the highest average log-likelihood.[^22] Then the average log-likelihood of a given predictive method's predictions provides a measure of the predictive accuracy of that method.[^23] If, on the other hand, there is no way to reliably predict the probability distribution of interest, then no predictive method is better than any other for achieving (or approximating) that purpose.
...
[^22]: This actually seems to provide a novel philosophical interpretation of probability, as discussed below in @sec-intprob.

[^23]: The phrase "predictive accuracy" here is used analogously to how it is defined in [@forster1994]: average log-likelihood, which can also be interpreted as a measure of closeness to the true probability distribution in terms of the Kullback-Liebler divergence, the average difference in log-likelihoods.

Again, what counts as "similar predictions" is specified as part of the definition of the prediction task at hand, the relevant prediction class. In addition, for probabilistic predictions, the $J$ prediction tasks for which the outcome is already known may be weighted according to their similarity to the task at hand, specifying $w_j$ for $j=1, ..., J$. Equal weights may be appropriate in certain circumstances, but generally not if the prediction task at hand is other than a random sample of the same distribution of tasks that generated the known outcomes. Then the weights should be used to better estimate the model's predictive accuracy for the task at hand, as discussed in an example with real data below.

::: {#def-success}
Given a prediction class and weights $w_j$ (with $\sum w_j = 1$), *predictive success* is $\sum_{j=1}^{J} w_j \cdot \log f(B_j \mid A_j)$, where f is the probability the predictive method assigns to $B_j$, given $A_j$. (This assumes the predictive method is fully-utilized with respect to the $J$ prediction tasks. If not, if a step or input in the predictive method can be identified as having no effect on any of the $J$ predictions, assign $-\infty$.)
:::

For our linear curve-fitting example, fitting a Gauss Linear model to the data will generally be the most successful predictive method. The method of instead, given *n* known data points, fitting a polynomial *p* of degree *n – 1* will fit the known data exactly but will likely fluctuate wildly up and down in between the known points and thus predict unknown data poorly. We can reject other alternative hypotheses in the same way, leaving the Gauss Linear model as the most successful predictive method.
...
Identifying the true data-generating process (given limited data) is not in itself our goal. Rather, the goal is making predictions as accurate as possible in the context at hand. In many cases, especially in the softer sciences, we have no hope of correctly specifying the true model anyhow.[^26] Let's look at an example using real data for which that is the case.

[^26]: See @sec-science for further discussion.

### Example: Predicting height by age and sex

Suppose we have data on age, sex, and height for a random sample of those aged 20-74 in a given population.[^27] And suppose the goal is to predict the height of someone aged 20-74, chosen at random.[^28] We consider a range of predictive methods, which each involve estimating, by ordinary least squares, a statistical model in which height is specified as a function of age and sex. Leave-one-out (LOO) cross-validation is the natural choice for quantifying predictive performance in this context, but it is shown alongside other standard measures in the table below.[^29] The most successful method is the specification ‘*female age age2*’. In other words, projecting height onto a linear combination of gender, age and age squared performs best for predicting height. So that is the model we should use to predict the height of someone aged 20-74 in this population.

[^27]: And suppose we have no other information to go on. The data for this example is Stata’s “nhanes2d” dataset, downsampled in such a way that each observation in the resulting sample has equal weight, to avoid the complications of accounting for stratified sampling.

[^28]: Use the (negative) mean squared error is the measure of success.

[^29]: The LOO R^2^ is calculated by predicting each observation using only the other observations to estimate the model (by OLS, here), then tallying up the squared prediction errors and expressing the average as an R^2^ (for now: really it should be predicted likelihood). It has been shown to be asymptotically equivalent to the AIC by @stone1977, explaining their equivalent rankings.)
...
The key commonality is that Reichenbach, while clearly acknowledging that using induction is not sufficient for success, argued that it is *necessary*. "I do not know whether an operation will save the man, but if there is any remedy, it is an operation."[^32]

[^32]: (Reichenbach 1938 \[2006: 349\]), as quoted in [@henderson2022].
...
Strevens’ book goes on to argue that the power of modern science stems from how it forces scientists to relentlessly prioritize tedious empirical observations over the aesthetic, religious, or philosophical considerations to which pre-modern students of the natural world dedicated the bulk of their attention and writing. But Strevens never returns to the basic question of how generalizing from all those meticulous observations can be justified in the first place.[^36] Strevens’ readers, despite his forceful presentation of Hume’s problem of induction in the first chapter, will probably have safely forgotten this disturbing quandary by the end of his subsequent thirteen chapters. But the justification of inductive reasoning presented here allows a proper appreciation of why all that careful observation Strevens celebrates matters: by eliminating rival predictive methods, we are left not with a bewildering infinitude of theories yet capable of fitting our still-finite observations but rather, for so many practical purposes, a single relevant predictive method, enabling our ever-expanding technical mastery.

[^36]: Strevens gestures toward “Baconian convergence” but does not attempt to explain why this seems to work so well rather than being defeated by the underdetermination that seems so inevitable when confirmation theory is considered in the abstract.
