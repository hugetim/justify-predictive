...
When a certain predictive method is not available, the following generalization of @prp-worked applies (and is likewise justified by @prp-same and our pragmatic need for prediction):
...
For our curve-fitting example, with predicted likelihood as the metric, fitting a linear model with a Gaussian error to the data should generally be the most successful predictive method. Generally speaking, though, the best predictive method for a given purpose will often not correspond to the true data-generating process. Since our specified aim is making predictions, though, identifying the true data-generating process need not concern us directly. In many cases, especially in the softer sciences, we have no hope of correctly specifying the true model anyhow.

* sine curve fitting example, in which linear is best (even without truly random noise)

* Then make point about generalization, maybe with age-height example
