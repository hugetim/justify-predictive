...
The problem of underdetermination is a central issue in the philosophical assessment of any form of inductive inference: multiple hypotheses can always be found which fit past data equally well, so how do (or should) we decide which to prefer? This section explains how thinking in terms of predictive methods, rather than hypotheses, largely alleviates the otherwise crippling issue of underdetermination. Whereas it is relatively easy to "fit" data after the fact, making accurate predictions is a true feat. Predictive track records provide a clear way to distinguish good from bad predictive methods, without appealing to simplicity or other secondary criteria.

To begin, though, let us clarify what is meant by a "predictive method." Consider that fully specifying the method used for any prediction, whether of the everyday or scientific variety, would generally be quite difficult. Even for an apparently basic prediction like "All ravens are black," to fully spell out how to determine whether something is a raven may be quite involved. Imagine trying to program a robot to do so.[^2] That is the kind of specificity required to properly define a "predictive method," the reproducible procedure used to make a given prediction.
...
::: {#def-predictive}
A *predictive method* is a procedure for using some input information $A$ to predict some outcome(s) $B$. Included are operational definitions of all relevant concepts and any tacit knowledge required to carry out the steps.
:::

Of course, the more common way to speak about "All ravens are black" is as a "hypothesis," a statement that is either true or false.[^3] By contrast, a predictive method does not have a truth value, per se, but rather a track record: whether its predictions have been successful. And in addition to its track record of literal predictions of then-unknown observations, one can also assess how well it *could have* predicted already-known data.[^4] In this "hold-out evaluation" approach, $m$ of the $N$ known outcomes are "held out" for evaluation, with the predictive method only allowed to use the remaining data, a "training set" of size $n=N-m$, as inputs for predicting the $m$ held-out outcomes.
...
Again, I am not saying that we *know* a successful method will continue to work for future predictions. There are no such guarantees.[^18] And when a previously successful method fails, we should revise it. But unless or until that happens, we ought to use a predictive method that has worked.

[^18]: In this sense Hume's problem of induction remains unsolved and presumably unsolvable. See @sec-hume.
