...
[^8]: For the uninitiated, this is Nelson Goodman’s "new riddle of induction."
...
The implications of a probabilistic predictive method's track record are not as clear-cut as that of a deterministic one. There will generally not be a unique method that is consistent with the predicted outcomes. And choosing the method with the best track record is not a necessary condition for choosing a reliable method.[^15] But using the most successful predictive method is nonetheless justified by an argument analogous to that given above for deterministic predictive methods.

[^15]: Indeed, as we shall see below, the most successful method will itself generally imply that another method may well have been chosen if the past predicted outcomes had turned out differently, if different values of the predicted probability distribution had been realized.

The idea is that, when a successful deterministic predictive method is unavailable, we still ought to act as if there were uniformity of nature in the following sense: that a predictive method exists which reliably indicates the true probabilities of the outcome in question.[^16] Here, "true" means the best probability distribution possible to predict (given the allowed inputs of the predictive task), where "best" means the highest average log-likelihood.[^17] Then the average log-likelihood of a given predictive method's predictions provides a measure of the predictive accuracy of that method.[^18] If, on the other hand, there is no way to reliably predict the probability distribution of interest, then no predictive method is better than any other for achieving (or approximating) that purpose.

[^16]: This is akin to the first of three assumptions @forster1994 enumerate for Akaike's Theorem: "a 'uniformity of nature' assumption that says that the true curve, whatever it is, remains the same for both the old and the new data sets considered in the definition of predictive accuracy."

[^17]: This actually seems to provide a novel philosophical interpretation of probability, as discussed below in @sec-intprob.

[^18]: The phrase "predictive accuracy" here is used analogously to how it is defined in [@forster1994]: average log-likelihood, which can also be interpreted as a measure of closeness to the true probability distribution in terms of the Kullback-Liebler divergence, the average difference in log-likelihoods.
...
The key commonality is that Reichenbach, while clearly acknowledging that using induction is not sufficient for success, argued that it is *necessary*. "I do not know whether an operation will save the man, but if there is any remedy, it is an operation."[^25]

[^25]: (Reichenbach 1938 \[2006: 349\]), as quoted in [@henderson2022].
