...
[^22]: The LOO R^2^ is calculated by predicting each observation using only the other observations to estimate the model (by OLS, here), then tallying up the squared prediction errors and expressing the average as an R^2^. It has been shown to be asymptotically equivalent to the AIC by @stone1977, explaining their equivalent rankings.)
...
If the data are approximately linear,[^25] a best-fit line will be able to predict well with relatively few known measurements available as inputs. The method of instead, given \_n \_known data points, fitting a polynomial \_p \_of degree \_n – 1 \_will fit the known data exactly but will likely fluctuate wildly up and down in between the known points and thus predict unknown data poorly.[^26] We can reject other alternative hypotheses in the same way, leaving the best-fit line as the most successful predictive method.[^27]

[^25]: To fix ideas, assume the true data-generating process is a linear function plus Gaussian noise.
