...
It will also be helpful to define a prediction problem more explicitly:

::: {#def-class}
A *prediction class* is a set of prediction tasks, inclusive of both the $J$ prediction tasks for which the outcome is already known and a potentially infinite number of similar tasks for which it is not. Each prediction task $j$ involves predicting outcome(s) $B_j$ using input information $A_j$.
:::
