...
## A justification for perfectly successful predictive methods {#sec-prag}

We never know whether a successful predictive method will continue working, but there is a justification for acting as if it will. For starters, let us consider the case of a predictive method with a perfect track record of categorical predictions, leaving to the next section the question of probabilistic predictive methods. And, for the moment, suppose that this method is uniquely successful, that there is no other known method that made or could have made the same predictions.[^ More precisely, suppose there are no "Case 2" alternatives in the sense discussed in the previous section.] Suppose also that its predictions are practically relevant to an impactful choice facing us.

- Maybe I should frame the question in terms of a well-defined set of past/potential predictions similar to a prediction of interest?

The justification begins by distinguishing two possibilities: either generally successful predictive methods exist or they don't. In the first case, adopting a so-far-successful method is a prerequisite for successful prediction [[But what about an as-yet-untried method?]] [15]
...
Another way to put @prp-worked is that 

assuming past performance is informative of future success: otherwise, prediction is simply not possible. Prior predictive performance is our only source of information about whether a predictive method will work going forward. The basis for this claim is empiricism: our only possible way to judge whether a method works is whether it has (or could have) worked in the past.[^12] So we have nothing to lose by assuming that it is possible to use known information to predict the unknown.
...
- As another example, consider an engineer designing a bridge. Is she justified in using techniques grounded in Newtonian physics to do so?[^ Within its scope, contexts in which both relativistic and quantum effects are negligible, let us posit that Newtonian mechanics has a (uniquely) flawless track record, hypothetically at least.] If a colleague disagrees with the design based on his interpretation of astrology, is there a solid philosophical reason to overrule him?
