...
\maketitle
\begin{abstract}
There is a pragmatic justification for using predictive methods that
have proven successful. Positing a specific form of uniformity is shown
to be necessary for predicting based on the available track record.
Approaching the available information in terms of what could have been
predicted---combined with a razor stripping candidate methods of
unutilized steps---is shown to eliminate the alternatives often thought
to make underdetermination inevitable. This combination provides a new
angle on Hume's problem of induction.
\end{abstract}
...
justification for the use of successful predictive methods. That is,
this justification of predictive methods is intended to be independent
of claims about the confirmation of broader theories---or the truth of
narrow inductive inferences---from which predictions may otherwise be
thought to derive. Simply stated, to predict an unknown outcome, one
should use a predictive method that has, or could have, been successful
at similar predictions.

This is about ``predictions'' broadly conceived, inclusive of statements
about the yet-unknown consequents of past events.\footnote{As Forster
...
practice. Other practioners (such as most economists, in my experience)
regard the use of cross-validation for model selection as an ad hoc
approach that could only be justified in terms of more traditional
statistics concepts, if at all.

Philosophers often seem to regard cross-validation as an ad hoc approach
that, at best, works because it approximates more philosophically
grounded criteria such as Akaike's Information Criterion
(AIC).\footnote{Partial exceptions that partially prove the rule include
  (Forster 2002) and (Hitchcock and Sober 2004).} More generally, as a
...
Nonetheless, there are some close parallels to this paper in the
philosophy literature. Perhaps most prominently in recent years, Gerhard
Schurz's meta-inductive program (Schurz 2021) justifies a predictive
strategy via optimality theorems from computational learning theory.
But, as I argue in Section~\ref{sec-schurz}, those theorems turn out to
provide no forward-looking, short-term warrant for adopting the
prescribed strategy.

Also of clear relevance is the literature summarized by {[}barnes2022{]}
debating why, or whether, novel predictions should receive more weight
than mere accomodation in assessing scientific theories. As explained
below, my own view is that the evidential value of pre-existing
``accomodated'' data becomes clearer once it is re-framed in terms of
whether it \emph{could have been} predicted.
...
of induction. Section~\ref{sec-schurz} contrasts this approach with
Schurz's meta-inductive program. A brief final section sketches the
interpretation of probability suggested by this framework.
...
genuine predictions thus provides a descriptive account of the intuition
...
predict other outcomes.\footnote{In some previous literature, the
  ...
  analogous to the family of all straight lines, as defined in (Forster
  and Sober 1994), for example (which notes that such families ``are of
  interest because they are \emph{instruments of prediction}.''
...
that pass through the 10 known data points is what Forster and Sober
...
weight to a precision of 0.1 lb. (and a range from 0 to 9.9). The result
...
predict her 9th measurement exactly.\footnote{Discretizing the x-axis to
  ...
  \(100^{3600 \cdot 24 - 9}=10^{172782}\) such functions that go through
  all nine points exactly.} On the other hand, there are many more
curves fitting the first eight points that will miss the 9th (99 times
more, in this discrete case\footnote{There are 100 possible values for
  each measurement, given the precision and range specified above}). The
former is how the situation looks from the standpoint of accomodation,
the latter from the perspective of prediction.
...
simply identify the dumbbell and predict 3 as its weight.\footnote{Note
  that the corresponding reply for \emph{predicates} does not work in
  the same way it does for predictive methods. In terms of predicates,
  there is a linguistic symmetry between ``thrine'' and ``three'':
  relative to the grue language, it is ``three'' that has a
  gerrymandered definition (thrine before 11pm, ``nee'' afterwards).
  Nothing about the the syntax of either language can break the tie. It
  is thinking in terms of procedures, the operations involved in
  applying a predictive method, that breaks the symmetry. A
  thrine-predictor must consult a clock on every task, comparing against
  the 11pm threshold, whereas the three-predictor performs no such
  operation.}

Implicit in the definition of predictive success, then, is a specific
form of ``Occam's razor'': if a step or input in a predictive method can
be identified as having no effect on any predictions it has (or could
have) made, it should not be considered part of the predictive method
that made those predictions. Rather, the predictive success should be
...
value can claim predictive success on its own. In this example,
predicting a constant value is the uniquely successful, fully-utilized
predictive method.
...
of the outcome in question.\footnote{This is akin to the first of three
  assumptions Forster and Sober (1994) enumerate for Akaike's Theorem:
  ``a `uniformity of nature' assumption that says that the true curve,
  whatever it is, remains the same for both the old and the new data
  sets considered in the definition of predictive accuracy.''} Here,
define ``true'' as the probability distribution that maximizes the
...
predictive accuracy of that method.\footnote{The phrase ``predictive
  accuracy'' here is used analogously to how it is defined in (Forster
  and Sober 1994): average (or ``expected'') log-likelihood, which can
  also be interpreted as a measure of closeness to the true probability
  distribution in terms of the Kullback-Leibler divergence, the average
  difference in log-likelihoods.} One ought to use the best method
according this measure because, according to the available information,
it is closest to indicating the true probabilities, if a reliable method
for predicting those probabilities exists. If, on the other hand, there
is no way to reliably predict the probability distribution of interest,
then no predictive method is better than any other for achieving that
purpose.
...
task presented in (Forster 2000). Suppose you observe 20 values of \(y\)
at each evenly spaced interval of \(x\) from 0 to 3.5, the values shown
in Figure~\ref{fig-known-data}, and you want to predict \(y\) for \(x\)
between 3.5 and 5.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-latex/notebooks-forster2000_adapted-fig-known-data-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-latex/notebooks-forster2000_adapted-fig-poly4-predict-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-latex/notebooks-forster2000_adapted-fig-poly4-eval-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-latex/notebooks-forster2000_adapted-fig-poly1-eval-output-1.png}}
...
for which predictive method should be used. Is this apparent
...
Barnes, Eric Christian. 2022. \emph{Prediction Versus Accommodation}.
Winter 2022. Edited by Edward N. Zalta and Uri Nodelman. Metaphysics
...
\bibitem[\citeproctext]{ref-forster2002}
Forster, Malcolm. 2002. {``Predictive Accuracy as an Achievable Goal of
Science.''} \emph{Philosophy of Science} 69 (S3): S124--34.
\url{https://doi.org/10.1086/341840}.

\bibitem[\citeproctext]{ref-forster2008}
Forster, Malcolm. 2008. \emph{Prediction}. Routledge.
...
Hájek, Alan. 2023. \emph{Interpretations of Probability}. Winter 2023.
Edited by Edward N. Zalta and Uri Nodelman. Metaphysics Research Lab,
Stanford University.
\url{https://plato.stanford.edu/archives/win2023/entries/probability-interpret/}.

\bibitem[\citeproctext]{ref-henderson2022}
Henderson, Leah. 2022. \emph{The Problem of Induction}. Winter 2022.
Edited by Edward N. Zalta and Uri Nodelman. Metaphysics Research Lab,
Stanford University.
\url{https://plato.stanford.edu/archives/win2022/entries/induction-problem/}.

\bibitem[\citeproctext]{ref-hitchcock2004}
Hitchcock, Christopher, and Elliott Sober. 2004. {``Prediction Versus
Accommodation and the Risk of Overfitting.''} \emph{The British Journal
for the Philosophy of Science} 55 (1): 1--34.
...
Romeijn, Jan-Willem. 2022. \emph{Philosophy of Statistics}. Fall 2022.
Edited by Edward N. Zalta and Uri Nodelman. Metaphysics Research Lab,
Stanford University.
...
Salmon, Wesley C. 2017. \emph{The Foundations of Scientific Inference:
50th Anniversary Edition}. University of Pittsburgh Press.
\url{https://www.jstor.org/stable/j.ctt1s475s3}.

\bibitem[\citeproctext]{ref-schurz2021}
Schurz, Gerhard. 2021. {``Meta-Induction over Unboundedly Many
Prediction Methods: A Reply to Arnold and Sterkenburg.''}
\emph{Philosophy of Science} 88 (2): 320--40.
\url{https://doi.org/10.1086/711588}.
